...
func state_process(delta:float):
	parent.anim_tree 
	parent.velocity = parent.Target * parent.momentun
	parent.parry_area.disabled = false
	var colision_info = parent.move_and_collide(parent.Target)
	parent.create_after_image()
	
	if colision_info:
		var tween = get_tree().create_tween()
		parent.Target = parent.Target.bounce(colision_info.get_normal())
		parent.parry_area.disabled = true
		parent.charge_level = 1.2
		tween.tween_property(parent,"momentun",0,1)
